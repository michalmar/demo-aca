              ...
              <Card className="bg-card/90 backdrop-blur">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-3 text-sm text-muted-foreground md:grid-cols-3">
                    <div className="rounded-lg border border-border/60 bg-background/80 p-3">
                      <p className="text-xs uppercase tracking-wider text-muted-foreground">{isFlashcard ? 'Reviewed' : 'Correct'}</p>
                      <p className="mt-1 text-lg font-semibold text-emerald-500">{correctCount}</p>
                    </div>
                    <div className="rounded-lg border border-border/60 bg-background/80 p-3">
                      <p className="text-xs uppercase tracking-wider text-muted-foreground">{isFlashcard ? 'Remaining' : 'Incorrect'}</p>
                      <p className="mt-1 text-lg font-semibold text-destructive">{isFlashcard ? totalQuestions - correctCount : incorrectCount}</p>
                    </div>
                    <div className="rounded-lg border border-border/60 bg-background/80 p-3">
                      <p className="text-xs uppercase tracking-wider text-muted-foreground">{isFlashcard ? 'Progress' : 'Score'}</p>
                ...
                <CardFooter className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <Button onClick={handleRetake} disabled={retaking} variant="secondary">
                    {retaking ? 'Preparing…' : isFlashcard ? 'Retake deck' : 'Retake questionnaire'}
                  </Button>
                </CardFooter>
              </Card>
            ) : (
              <>
                {!loading && questions.length > 0 && (
                  <Card className="mb-6 bg-card/80 backdrop-blur">
                    <CardHeader className="pb-3">
                      <CardTitle className="text-sm font-semibold">{isFlashcard ? 'Card Progress' : 'Answer Progress'}</CardTitle>
                    </CardHeader>
                    <CardContent className="flex flex-col gap-3">
                      <div className="flex flex-wrap gap-2">
                        {answeredStates.map((state, index) => {
                          ...
                          const labelBase = state === 'correct'
                            ? isFlashcard ? 'reviewed' : 'correct'
                            : state === 'incorrect'
                              ? 'incorrect'
                              : isFlashcard ? 'not reviewed' : 'pending';
                          return (
                            <Badge
                ...
                {!loading && currentQuestion && (
                  <p className="text-sm text-muted-foreground">
                    {isFlashcard ? 'Flip each card to reveal the explanation.' : 'Answer the prompt below to continue.'}
                  </p>
                )}
              </>
            )}
